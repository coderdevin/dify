...
cd ../api && \
docker build . -t langgenius/dify-api:$tag_built --build-arg COMMIT_SHA=$ref_after
# build docker image
echo "$changed_files" | grep -q "^web$" && \
cd ../web && \
docker build . -t langgenius/dify-web:$tag_built --build-arg COMMIT_SHA=$ref_after
